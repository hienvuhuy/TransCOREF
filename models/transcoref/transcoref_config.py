...
# from fairseq.models.transformer_clean.transformer_clean_config import (
#     EncDecCleanBaseConfig, DecoderCleanConfig, QuantNoiseCleanConfig, TransformerCleanConfig
# )
...
@dataclass
class EncDecTransCOREFBaseConfig(FairseqDataclass):
...
@dataclass
class DecoderTransCOREFConfig(EncDecTransCOREFBaseConfig):
...
@dataclass
class QuantNoiseTransCOREFConfig(FairseqDataclass):
    pq: float = field(
        default=0.0,
...
@dataclass
class CorefRepresentationTransCOREFConfig(EncDecTransCOREFBaseConfig):
    input_dim: int = II("model.coref_representations.embed_dim")
    output_dim: int = field(
        default=II("model.coref_representations.embed_dim"),
        metadata={
            "help": "coref-representations output dimension (extra linear layer if different from bert-representations embed dim)"
        },
    )

    def __post_init__(self):
        #  II doesn't work if we are just creating the object outside of hydra so fix that
        if self.input_dim == II("model.coref_representations.embed_dim"):
            self.input_dim = self.embed_dim
        if self.output_dim == II("model.coref_representations.embed_dim"):
            self.output_dim = self.embed_dim

@dataclass
class CorefModuleProbingConfig(FairseqDataclass):
    # max_span_length int 10
    max_span_length: int = field(
        default=10,
        metadata={"help":"maximum tokens for a span"}
    )
    
    # top_lambda float 0.4
    top_lambda: float = field(
        default=0.4,
        metadata={"help":"threshold value for select top highest score of possible spans"}
    )

    # ffn_embed_dim int 2048 ; in original paper of coref, values is 3072
    # ffn_embed_dim: int = field(
    #     default=2048, metadata={"help": "embedding dimension for FFN"}
    # )
    ffn_embed_dim: int = field(
        default=512, metadata={"help": "embedding dimension for FFN"}
    )
    # normalise_loss bool True
    normalise_loss: bool = field(
        default=True, metadata={"help": "apply normalized loss"}
    )    
    # hidden_size int 512
    hidden_size: int = field(
        default=512, metadata={"help": "hidden size"}
    )
    # hidden_size: int = field(
    #     default=2048, metadata={"help": "hidden size"}
    # )
    # dropout_prob
    ...
    layer_norm_eps: float = field(
        default=1e-5,
        metadata={"help":"layer norm eps value"}
    )
    pass

@dataclass
class TransCOREFConfig(FairseqDataclass):
    ...
    encoder: EncDecTransCOREFBaseConfig = EncDecTransCOREFBaseConfig()
    ...
    decoder: DecoderTransCOREFConfig = DecoderTransCOREFConfig()
    # TODO should really be in the decoder config
    
    # for coref_representation
    coref_representations: CorefRepresentationTransCOREFConfig = CorefRepresentationTransCOREFConfig()

    # for coref_module
    coref_module: CorefModuleProbingConfig = CorefModuleProbingConfig()
    ...
    quant_noise: QuantNoiseTransCOREFConfig = field(default=QuantNoiseTransCOREFConfig())
    ...
    ## for the additional bert layer and coref
    coref_top_lambda: float = field(
        default=0.4,
        metadata={'help': 'pruning value for keeping span in coref model'}
    )
    coref_max_span_length: int = field(
        default=10,
        metadata={'help': 'maximum span lengths (smilar to sliding window)'}
    )
    coref_hidden_act: str = field(
        default= 'gelu',
    )
    coref_normalize_loss: bool = field(
        default=True,
    )
    coref_layer_norm_eps: float = field(
        default=0.000001
    )
            ...
            for fld in fields(cls):
                ...
                if fld.name == "decoder":
                    if hasattr(args, "decoder"):
                        ...
                        seen.add("decoder")
                        config.decoder = DecoderTransCOREFConfig(**args.decoder)
                    else:
                        config.decoder = cls._copy_keys(
                            args, DecoderTransCOREFConfig, "decoder", seen
                        )
                elif fld.name == "encoder":
                    # same but for encoder
                    if hasattr(args, "encoder"):
                        seen.add("encoder")
                        config.encoder = EncDecTransCOREFBaseConfig(**args.encoder)
                    else:
                        config.encoder = cls._copy_keys(
                            args, EncDecTransCOREFBaseConfig, "encoder", seen
                        )
                elif fld.name == "quant_noise":
                    # same but for quant_noise
                    if hasattr(args, "quant_noise"):
                        seen.add("quant_noise")
                        config.quant_noise = QuantNoiseTransCOREFConfig(**args.quant_noise)
                    else:
                        config.quant_noise = cls._copy_keys(
                            args, QuantNoiseTransCOREFConfig, "quant_noise", seen
                        )
