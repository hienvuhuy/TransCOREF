...
from torch.nn import Module, LayerNorm, Dropout, Parameter, Linear
...
from TransCOREF.utils import trans_coref_utils
...
class S2ECoref(nn.Module):
    ...
    def _get_cluster_labels_after_pruning_empty_of_all_clusters(self, span_starts, span_ends, all_clusters):
        """
        ...
        for b, (starts, ends, gold_clusters) in enumerate(zip(span_starts.cpu().tolist(), span_ends.cpu().tolist(), all_clusters_cpu)):
            gold_clusters = trans_coref_utils.extract_clusters(gold_clusters)
            mention_to_gold_clusters = trans_coref_utils.extract_mentions_to_predicted_clusters_from_clusters(gold_clusters)
    ...
    def _get_marginal_log_likelihood_loss_for_generation_step_old(self, coref_logits, cluster_labels_after_pruning, span_mask):
        """
        :param coref_logits: [batch_size, max_k, max_k]
        :param cluster_labels_after_pruning: [batch_size, max_k, max_k]
        :param span_mask: [batch_size, max_k]
        ...
        gold_log_probs = all_log_sum_exp
        
        losses = - gold_log_probs
        losses = losses * span_mask
        per_example_loss = torch.sum(losses, dim=-1)  # [batch_size]
        return per_example_loss + 0.00001 # to not face with 0 

    def _get_marginal_log_likelihood_loss_for_generation_step(self, coref_logits, cluster_labels_after_pruning, span_mask):
        """
        :param coref_logits: [batch_size, max_k, max_k]
        :param cluster_labels_after_pruning: [batch_size, max_k, max_k]
        :param span_mask: [batch_size, max_k]
        ...
        """
            Calculate score base on coref_logits
            Formulation:
                log(P(C|x,y))= log [ \prod_{c \in C} { p(c|s) } ]
                    = log [exp( sum_{c \in C }{f(c|s))} /(\sum{c \in C}{exp(f(c|s))})^{|C|} ] 
                    = log [exp( sum_{c \in C }{f(c|s))}] - log[(\sum{c \in C}{exp(f(c|s))})^{|C|}]
                    = sum_{c \in C }{f(c|s))} - |C|*log[(\sum{c \in C}{exp(f(c|s))})]
                    = torch.sum(coref_logits) - |C|*torch.logsumexp(coref_logits, dim=-1)

            Loss:
                log(sum_{c \in C_{golden} } P(c|s)) = log( sum_{c \in C_{golden}} {exp(f(c, s))/(sum_{c' \in C}{exp(f(c',s))})})
                    = log([sum_{c \in C_{golden}} {exp(f(c, s))]/[(sum_{c' \in C}{exp(f(c',s))})] )
                    = log([sum_{c \in C_{golden}} {exp(f(c, s))]) - log([(sum_{c' \in C}{exp(f(c',s))})])
                    = log(torch.logsumexp(gold_coref_logits, dim=-1)) 
        """
        gold_coref_logits = mask_tensor(coref_logits, cluster_labels_after_pruning)
        ...
        bs, max_k, max_k_1 = coref_logits.shape
        numerator = torch.sum(coref_logits, dim=-1)
        denominator = torch.logsumexp(coref_logits, dim=-1)
        probs = numerator - max_k*denominator # [batch_size, max_k]
        probs = torch.sum(probs, dim=-1)  # [batch_size]
        return probs
    ...
    def forward(self, input_representation, gold_clusters=None, loss_to_generate=False):
        ...
        outputs = input_representation[0]['coref_representations_out']
        ...
        if gold_clusters is not None:
            assert gold_clusters.shape[0] == sequence_output.shape[0]
        ...
        if gold_clusters is not None:
            ...
            if loss_to_generate:
                loss = self._get_marginal_log_likelihood_loss_for_generation_step(final_logits, labels_after_pruning, span_mask)
            else:
            ...
            labels_after_pruning = self._get_cluster_labels_after_pruning_empty_of_all_clusters(mention_start_ids, mention_end_ids, gold_clusters)
            loss = self._get_marginal_log_likelihood_loss(final_logits, labels_after_pruning, span_mask)
            return {

                # "coref_probing_output":(None, mention_start_ids, mention_end_ids, final_logits, mention_logits, None),
                # "coref_loss": None
                # edit on 2023, Feb 05th
                "coref_probing_output":(labels_after_pruning, mention_start_ids, mention_end_ids, final_logits, mention_logits, span_mask),
                "coref_loss": loss
